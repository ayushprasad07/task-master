...
export function AnimatedModal() {
  ...
  const [credentials, setCredentials] = useState({title:"", description: "", url:""});
  ...
  const handleSubmit = async(e : React.FormEvent<HTMLFormElement>) => {
    ...
    setLoading(true);
    try {
      const {title, description, url} = credentials;
      const response = await  axios.post("/api/create-youtube-note",{
          title,
          description,
          url
      })
          ...
          <ModalBody>
            <ModalContent>
              ...
              <div className="py-4 flex flex-wrap gap-x-4  items-start justify-start max-w-sm mx-auto">
                ...
                <Input id="url" name="url" placeholder="" type="text" onChange={handleChange} />
                </LabelInputContainer>
              </div>
            </ModalContent>
